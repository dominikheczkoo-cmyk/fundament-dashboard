...
import {
  FLAG, MAIN_CUR, KOTACE, korelace, vahaKat, vahaTypu, jePodstatne, platiProMenu, kategorieUdalosti,
  czDate, shortDate, denVTydnu, cas,
} from "../app/lib-ui";
...
function fundamentPodleMen(events) {
  ...
  events.forEach((e) => {
    ...
    // změna zaměstnanosti jen za celou eurozónu, ne za jednotlivé země
    if (!platiProMenu(kat, e.cur)) return;
    ...
    // předběžná data váží víc než finální revize, kterou trh už zná
    const v = vahaKat(kat) * vahaTypu(e.typ) * (e.verdict === "+" ? 1 : -1);
...
function katalyzatoryPodleMen(events) {
  ...
  events.forEach((e) => {
    ...
    if (!jePodstatne(kat) || !platiProMenu(kat, e.cur)) return;
    const cur = e.cur;
    if (!cur || !MAIN_CUR.includes(cur)) return;
    const v = Math.round(vahaKat(kat) * vahaTypu(e.typ) * 10) / 10;
    if (v === 0) return;
    const o = (out[cur] = out[cur] || { suma: 0, udalosti: [] });
    o.suma += v;
...
export function duvody(p) {
  ...
  [a, b].forEach((s) => {
    ...
    const top = s.kat.udalosti.slice(0, 4)
      .map((e) => `${denVTydnu(e.date).slice(0, 2)} ${shortDate(e.date)}${cas(e.date) ? " " + cas(e.date) : ""} ${e.info} (váha ${e.vaha})`)
      .join("\n");
